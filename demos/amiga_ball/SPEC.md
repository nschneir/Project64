...
|---|---|---|---|---|---|---|---|---|---|
| y | 158.0 | 112.5 | 80.0 | 60.5 | 54.0 | 60.5 | 80.0 | 112.5 | 151.602 |

Stored as **8.8 fixed point**, 64 entries × 2 bytes = 128 bytes (`bounce.inc`),
generated by `tools/gen_bounce.py`. Impact is the wrap from `p = 63` to `p = 0`.
The table is **exact** in 8.8 — `y(p) × 256 = 40448 − 26·p·(64−p)` is an
integer for every `p`, so there is no rounding anywhere in it and the symmetry
check is an equality rather than a tolerance.

(An earlier draft of this table printed 79.0 and 57.5 at `p` = 16 and 24. Both
were wrong, and the table gave itself away by being internally inconsistent:
79.0 implies an amplitude of 105.33 and 57.5 implies 107.20, so no single `A`
produces both. The formula, the generator and the machine all agree on 80.0 and
60.5 — `bounce_phase` = 48 reads back `ball_yi` = 80. Caught by the Task 3
implementer.)
