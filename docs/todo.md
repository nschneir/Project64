...
**Anchor:** `demos/1812/sections.s:102` (cites `SPEC.md` §6.4),
`demos/1812/test.yaml:78` (cites acceptance criterion **A15**),
`SPEC.md:376`, `SPEC.md:397-398`, `AUDIT.md:174,181`.

**Status:** open. The amendments are the evidence-and-prose plan's deliverable;
this item exists because the code landed first.

**What's wrong now.** `sections.s:102` points at `SPEC.md` §6.4 for the texture
arc while `SPEC.md:372-375` still describes the old three-pad hymn.
`test.yaml:78` cites **A15**, which does not exist — `SPEC.md` §12 stops at A14.
`SPEC.md:397-398` still says "112-frame intervals", the exact naive-duration
error corrected in `music.s`. `AUDIT.md:172` still carries the measured
frame-40 hymn row from iteration 2.
...
`demos/1812/sections.s:145`.

**Status:** open, all three pre-existing.

**What's wrong now.** `vars.s:81-82` and `raster.s:272` both say "under 100
bytes of headroom"; the real figure was 252 before iteration 3 and is 184
after it. `sections.s:145` says "the anacrusis is 16-frame notes" — naive
...
comments match; for `sections.s:145`, confirm against `voicetick`'s countdown.
