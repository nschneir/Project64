...
def c64_test_run(yaml_file: str, allow_stale: bool = False) -> dict:
    ...
    sibling .lbl, a `program:` .prg newer than its own. `allow_stale` waives
    that (the timestamps can lie: `cp -r` without `-p` restamps a whole tree)
    and names what it let through in the returned `warnings`."""
